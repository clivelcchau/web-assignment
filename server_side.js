      ...
      console.log("signup");
						...
						res.writeHead(200,
            {
						...
						MongoClient.connect(dbUrl, function(err, db) 
						{
							...
							var dbo = db.db("webdb");
							var myobj = stringMsg;
							dbo.collection("member").count({"Name" : splitName[1]}, function(err, count)
							{
				...
				return req.on('data', function(data) 
				{
					...
					return req.on('end', function() 
					{
						...
						MongoClient.connect(dbUrl, function(err, db) 
						{
							...
							var dbo = db.db("webdb");
							var myobj = stringMsg;
							dbo.collection("member").count({"Name" : splitName[1], "Password" : splitPassword[1]}, function(err, count)
							{
								console.log(err, count);
								finalcount = count;
								if(err) throw err;
		...
		else if (action === "/loadflist")
		{
			...
			else
			{
				...
				MongoClient.connect(dbUrl, function(err, db) 
				{
					...
					var dbo = db.db("webdb");
					var myobj = stringMsg;
					dbo.collection("favour").find({"user" : loginUser}).toArray(function(err, result) 
					{
    				if (err) throw err;
    				console.log(result);
    				db.close();
					...
					return req.on('data', function(data) 
					{
						...
						return req.on('end', function() 
						{
							...
							MongoClient.connect(dbUrl, function(err, db) 
							{
								...
								dbo.collection("favour").count(myobj, function(err, count)
								{
									console.log(err, count);
									finalcount = count;
									if(finalcount > 0)
									...
									{
										dbo.collection("favour").insertOne(myobj, function(err, res) 
										{
											if (err) throw err;
						...
						return req.on('end', function() 
						{
							...
							MongoClient.connect(dbUrl, function(err, db) 
							{
								...
								dbo.collection("favour").count(myobj, function(err, count)
								{
									...
									if(finalcount > 0)
									{
										dbo.collection("favour").deleteOne(myobj, function(err, res) 
										{
											if (err) throw err;
        ...
        if(loginStatus)
				{
					sendFileContent(res, "login_index.html", "text/html");
				}
				else
				{
			...
			{
				sendFileContent(res, "sign_up_page.html", "text/html");
			}
			else if (req.url === "/page_login")
			{
				sendFileContent(res, "login_page.html", "text/html");
			}
			...
			else if (req.url === "/hkadagainstdrug")
			{
				sendFileContent(res, "text_hkadagainstdrug.html", "text/html");
			}
      else if (req.url === "/hkadpublicity")
			{
				sendFileContent(res, "text_hkadpublicity.html", "text/html");
			}
      else if (req.url === "/hkadyouthreso")
			{
				sendFileContent(res, "text_hkadyouthreso.html", "text/html");
			}
      else if (req.url === "/flistpage")
			{
				sendFileContent(res, "my_favourite.html", "text/html");
			}
			else if(req.url === "/")
			{
